...
import { getFinalApprovals, updateFinalApprovalStatus } from "@/app/actions/final-approvals";
...
type RequestType = Awaited<ReturnType<typeof getFinalApprovals>>[0];
...
export default function FinalApprovalsPage() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    const data = await getFinalApprovals();
    setRequests(data);
    setLoading(false);
  };
  ...
  const handleAction = async (requestId: number, status: string, customRejectionReason?: string) => {
    ...
    if (!reason) {
      const result = await Swal.fire({
        ...
        text: `คุณต้องการไม่อนุมัติคำขอใช้รถและปฏิเสธคำขอใช้รถนี้ใช่หรือไม่?`,
        input: 'textarea',
        inputPlaceholder: 'ระบุเหตุผลการปฏิเสธคำขอใช้รถ...',
        inputAttributes: {
    ...
    await updateFinalApprovalStatus(requestId, status, reason);
    Swal.fire({
      title: "สำเร็จ",
      text: `ทำรายการปฏิเสธคำขอใช้รถเรียบร้อยแล้ว`,
      icon: "success",
      timer: 1500,
  ...
  const handleSignatureConfirm = async (signatureOption: "saved" | "fresh", freshBase64Data?: string) => {
    ...
    // 1. Save digital signature for Director
    await saveApproverSignature(selectedRequestId, "director", signatureOption, freshBase64Data);

    // 2. Finalize request status to Approved
    await updateFinalApprovalStatus(selectedRequestId, "Approved");

    Swal.fire({
      title: "สำเร็จ",
      text: `คำขอได้รับอนุมัติขั้นสุดท้ายและลงลายมือชื่อเรียบร้อยแล้ว ระบบจะทำการแจ้งเตือนผู้ใช้และพนักงานขับรถ`,
      icon: "success",
      timer: 2000,
      showConfirmButton: false
    });
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight text-slate-800 flex items-center gap-2">
            <span className="h-6 w-1.5 rounded-full bg-gradient-to-b from-pink-500 to-rose-500"></span>
            อนุมัติขั้นสุดท้าย
          </h2>
          <p className="text-sm text-slate-500 mt-1">รายการขออนุญาตใช้รถที่ผ่านการพิจารณาและจัดคิวรถเรียบร้อยแล้ว รอการอนุมัติขั้นสุดท้าย</p>
        </div>
        <Badge variant="outline" className="bg-pink-50/50 text-pink-700 border-pink-200/50 px-3 py-1 text-xs font-semibold rounded-full shadow-sm">
          ผู้อำนวยการ
        </Badge>
      </div>
        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow>
              ...
              <TableHead className="font-semibold text-slate-700">รถยนต์ที่จัดให้</TableHead>
              <TableHead className="font-semibold text-slate-700">คนขับรถ</TableHead>
            ...
            {loading ? (
              ...
              requests.map((req) => (
                <TableRow key={req.requestId} className="hover:bg-slate-50/30 transition-colors">
                  ...
                  <TableCell>
                    <div className="font-bold text-pink-700">{req.car?.licensePlate || "-"}</div>
                    {req.car?.carType && (
                  ...
                  <TableCell>
                    <div className="font-bold text-pink-700">{req.driver?.driverName || "-"}</div>
                    {req.driver?.phone && (
                      <div className="text-xs text-slate-400 font-light mt-0.5">โทร: {req.driver.phone}</div>
                    )}
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="bg-pink-50 text-pink-600 border-pink-200/50 rounded-full font-medium">
                      รอ ผอ. อนุมัติ
                    </Badge>
                  </TableCell>
                  <TableCell className="space-x-2 whitespace-nowrap text-right">
                    ...
                    <Button size="sm" className="bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white rounded-xl shadow-sm hover:shadow transition-all" onClick={() => handleAction(req.requestId, "Approved")}>
                      อนุมัติขั้นสุดท้าย
                    </Button>
                    <Button 
                      variant="outline" 
            ...
            >
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-50 to-rose-50 border border-pink-100/50 flex items-center justify-center text-pink-600 font-bold text-sm">
                ...
                <Badge variant="outline" className="bg-pink-50 text-pink-600 border-pink-200/50 text-[10px] font-semibold px-2 py-0.5 rounded-full">
              ...
              <div className="space-y-2.5 pt-1">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  {/* Car Box */}
                  <div className="flex items-start gap-2 bg-pink-50/30 p-2.5 rounded-xl border border-pink-100/30">
                    <Car className="w-3.5 h-3.5 text-pink-600 shrink-0 mt-0.5" />
                    <div className="text-[10px] leading-tight">
                      <span className="font-bold text-slate-800 block">ทะเบียนรถ</span>
                      <span className="text-pink-700 font-bold block mt-0.5">{req.car?.licensePlate || "-"}</span>
                      {req.car?.carType && (
                  ...
                  {/* Driver Box */}
                  <div className="flex items-start gap-2 bg-rose-50/30 p-2.5 rounded-xl border border-rose-100/30">
                    <Users className="w-3.5 h-3.5 text-rose-600 shrink-0 mt-0.5" />
                    <div className="text-[10px] leading-tight">
                      <span className="font-bold text-slate-800 block">พนักงานขับ</span>
                      <span className="text-rose-700 font-bold block mt-0.5">{req.driver?.driverName || "-"}</span>
                      {req.driver?.phone && (
              ...
              {activeRejectionId !== req.requestId ? (
                <div className="grid grid-cols-3 gap-2.5 pt-2">
                  ...
                  <button 
                    ...
                    className="col-span-2 flex items-center justify-center gap-1.5 py-3 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-600 text-white font-extrabold text-xs shadow-md shadow-pink-100/50 hover:from-pink-600 hover:to-rose-700 active:scale-[0.98] transition-all"
                  >
                ...
                <div className="space-y-3 pt-2 bg-slate-50/60 p-3.5 rounded-2xl border border-red-100/50 animate-in fade-in slide-in-from-top duration-300">
                  ...
                  <textarea
                    ...
                    placeholder="กรอกเหตุผลที่ปฏิเสธคำขอใช้รถนี้..."
                      ...
                      onClick={() => {
                        if (!rejectionReasonText.trim()) {
                          Swal.fire({
                            ...
                            text: "กรุณากรอกเหตุผลเพื่อยืนยันการไม่อนุมัติครับ",
                            icon: "warning",
                            confirmButtonColor: "#dc2626"
                          });
                          return;
      ...
      <SignatureApprovalDialog
        ...
        title="อนุมัติขั้นสุดท้าย (ผู้อำนวยการ)"
        description="กรุณาเลือกลงนามเพื่ออนุมัติการเดินทางและออกใบอนุญาตการใช้งานรถราชการ"
        actionLabel="อนุมัติและลงนามขั้นสุดท้าย"
      />
    </div>
  );
}
