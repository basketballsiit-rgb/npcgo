...
import { getPendingApprovals, updateRequestStatus, saveApproverSignature, getCurrentUserRole } from "@/app/actions/approvals";
import { deleteVehicleRequest } from "@/app/actions/requests";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { format } from "date-fns";
import { th } from "date-fns/locale";
import { Printer, User, MapPin, Calendar, MessageSquare, ClipboardList, Check, X, ShieldAlert, Trash2 } from "lucide-react";
import Swal from "sweetalert2";
import { SignatureApprovalDialog } from "@/components/ui/signature-approval-dialog";

type RequestType = Awaited<ReturnType<typeof getPendingApprovals>>[0];
...
export default function ApprovalsPage() {
  ...
  const handleDeleteRequest = async (requestId: number) => {
    const result = await Swal.fire({
      ...
      text: `คุณแน่ใจหรือไม่ที่จะลบใบขอใช้รถเลขที่ #${requestId}? การกระทำนี้ไม่สามารถย้อนกลับได้`,
      icon: "warning",
    ...
    try {
      const res = await deleteVehicleRequest(requestId);
      if (res.success) {
        Swal.fire({
          title: "ลบสำเร็จ",
  ...
  const handleAction = async (requestId: number, status: string, customRejectionReason?: string) => {
    ...
    if (!reason) {
      const result = await Swal.fire({
        ...
        text: `คุณต้องการไม่อนุมัติคำขอใช้รถนี้ใช่หรือไม่?`,
        input: 'textarea',
        inputPlaceholder: 'ระบุเหตุผลการไม่อนุมัติคำขอ...',
        inputAttributes: {
          'aria-label': 'ระบุเหตุผลการไม่อนุมัติ'
        },
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#dc2626",
        ...
        inputValidator: (value) => {
          if (!value || value.trim() === "") {
            return 'กรุณากรอกเหตุผลการไม่อนุมัติด้วยครับ';
          }
        }
      });
    ...
    Swal.fire({
      title: "สำเร็จ",
      text: `ทำรายการไม่อนุมัติเรียบร้อยแล้ว`,
      icon: "success",
      timer: 1500,
      showConfirmButton: false
    });
  ...
  const handleSignatureConfirm = async (signatureOption: "saved" | "fresh", freshBase64Data?: string) => {
    ...
    // 1. Save digital signature first
    await saveApproverSignature(selectedRequestId, "deputy", signatureOption, freshBase64Data);
    
    // 2. Update status
    await updateRequestStatus(selectedRequestId, "Pending_Scheduler");
    
    Swal.fire({
      title: "สำเร็จ",
      text: `พิจารณาอนุมัติและลงนามเรียบร้อยแล้ว`,
      icon: "success",
      timer: 1500,
      showConfirmButton: false
    });
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight text-slate-800 flex items-center gap-2">
            <span className="h-6 w-1.5 rounded-full bg-gradient-to-b from-teal-500 to-emerald-500"></span>
            พิจารณาอนุมัติคำขอ
          </h2>
          <p className="text-sm text-slate-500 mt-1">รายการขออนุญาตใช้รถที่รอการพิจารณาจากรองผู้อำนวยการ</p>
        </div>
        <Badge variant="outline" className="bg-teal-50/50 text-teal-700 border-teal-200/50 px-3 py-1 text-xs font-semibold rounded-full shadow-sm">
          รองผู้อำนวยการ
        </Badge>
      </div>
      ...
      <div className="hidden md:block rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow>
              <TableHead className="font-semibold text-slate-700">วันที่ขอ</TableHead>
              <TableHead className="font-semibold text-slate-700">ผู้ขอ</TableHead>
              <TableHead className="font-semibold text-slate-700">สถานที่/วัตถุประสงค์</TableHead>
            ...
            {loading ? (
              ...
              requests.map((req) => (
                <TableRow key={req.requestId} className="hover:bg-slate-50/30 transition-colors">
                  <TableCell className="text-slate-600">{format(new Date(req.createdAt), "d MMM yyyy", { locale: th })}</TableCell>
                  <TableCell className="font-semibold text-slate-700">{req.requesterName}</TableCell>
                  <TableCell>
                    <div className="font-bold text-slate-800">{req.destination}</div>
                    <div className="text-xs text-slate-500 mt-0.5">{req.purpose}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm font-medium text-slate-700">{format(new Date(req.startDateTime), "d MMM yyyy HH:mm", { locale: th })}</div>
                    <div className="text-xs text-slate-400 mt-0.5">ถึง {format(new Date(req.endDateTime), "d MMM yyyy HH:mm", { locale: th })}</div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-200/50 rounded-full font-medium">
                      รออนุมัติ
                    </Badge>
                  </TableCell>
                  <TableCell className="space-x-2 whitespace-nowrap text-right">
                    ...
                    <Button size="sm" className="bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white rounded-xl shadow-sm hover:shadow transition-all" onClick={() => handleAction(req.requestId, "Pending_Scheduler")}>
                      อนุมัติ
                    </Button>
                    <Button 
                      variant="outline" 
            ...
            >
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100/50 flex items-center justify-center text-teal-600 font-bold text-sm">
                ...
                <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-200/50 text-[10px] font-semibold px-2 py-0.5 rounded-full">
                  รออนุมัติ
                </Badge>
              </div>
              ...
              <div className="space-y-2.5 pt-1">
                ...
                <div className="flex items-center gap-2.5 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100">
                  ...
                  <span className="text-[11px] text-slate-600">
                    ผู้เดินทาง <span className="font-bold text-slate-800">{req.passengerCount}</span> ท่าน ({req.passengerNames.substring(0, 30)}{req.passengerNames.length > 30 ? '...' : ''})
                  </span>
                </div>
              </div>
              ...
              {activeRejectionId !== req.requestId ? (
                <div className="grid grid-cols-3 gap-2.5 pt-2">
                  ...
                  <button 
                    onClick={() => handleAction(req.requestId, "Pending_Scheduler")}
                    className="col-span-2 flex items-center justify-center gap-1.5 py-3 rounded-2xl bg-gradient-to-r from-teal-500 to-emerald-600 text-white font-extrabold text-xs shadow-md shadow-emerald-100/50 hover:from-teal-600 hover:to-emerald-700 active:scale-[0.98] transition-all"
                  >
                ...
                <div className="space-y-3 pt-2 bg-slate-50/60 p-3.5 rounded-2xl border border-red-100/50 animate-in fade-in slide-in-from-top duration-300">
                  ...
                  <textarea
                    ...
                    placeholder="กรอกเหตุผล เช่น ข้อมูลไม่ครบถ้วน, ติดภารกิจด่วนอื่น..."
                      ...
                      onClick={() => {
                        if (!rejectionReasonText.trim()) {
                          Swal.fire({
                            ...
                            text: "กรุณากรอกเหตุผลเพื่อใช้อ้างอิงการไม่อนุมัติครับ",
                            icon: "warning",
                            confirmButtonColor: "#dc2626"
                          });
                          return;
              ...
              {/* Open in print sheet button & Admin Delete button */}
              <div className="flex justify-between items-center pt-2 border-t border-slate-50 mt-1">
                <div>
                  {isAdmin && (
                    <button 
                      onClick={() => handleDeleteRequest(req.requestId)}
                      className="flex items-center gap-1 text-[10px] text-red-500 hover:text-red-700 px-2.5 py-1.5 rounded-lg border border-red-100 hover:bg-red-50/30 transition-colors font-semibold"
                    >
      ...
      <SignatureApprovalDialog
        ...
        title="ลงนามอนุมัติ (รองผู้อำนวยการ)"
        description="กรุณาเลือกลงนามเพื่ออนุมัติคำขอใช้รถยนต์และส่งเรื่องต่อไปยังงานยานพาหนะ"
      />
    </div>
  );
}
